...
//
//			// ========== RESTORE ADC CALIBRATION FROM DEFAULTS ==========
//			ADC_Restore_Calibration_From_Config(&device_config);
//			break;
//
//		default:
...
//		    break;
			 ...
			 if (key == KEY_UP) {
			    ...
			    break;
		case MODE_OUTPUT_CAL_LOW:
//		    // Enable bypass mode - stops temperature from affecting PWM
//		    AnalogOutput_Calibration_Bypass(true);
//
//		    // Adjust PWM value with keys
//		    if (key == KEY_UP) {
//		        device_config.cal_output_lo++;
//		        if (Keyboard.Fast_Value_Change) {
//		            Fast_value_Inc();
...
//		    }
//
//		    // Clamp to 4-20mA range (200-999)
////		    if (device_config.cal_output_lo < 200) device_config.cal_output_lo = 200;
//		    if (device_config.cal_output_lo > 2000) device_config.cal_output_lo = 2000;
//
//		    // Output RAW PWM value directly (bypass temperature scaling)
...
//
//		    // Clamp to 4-20mA range (200-999)
////		    if (device_config.cal_output_hi < 200) device_config.cal_output_hi = 200;
...
//		   case MODE_PWM_TEST:
//		        {
//		            static uint16_t pwm_test_value = 500;  // Start at mid-range
//
//		            // Enable bypass mode to control PWM directly
//		            AnalogOutput_Calibration_Bypass(true);
//
//		            // Adjust PWM with UP/DOWN keys
//		            if (key == KEY_UP) {
//		                pwm_test_value++;
//		                if (Keyboard.Fast_Value_Change) {
//		                    Fast_value_Inc();
...
//		            if (pwm_test_value > 999) pwm_test_value = 999;
//		            if (pwm_test_value < 0) pwm_test_value = 0;
//
//		            // Output the PWM value
//		            AnalogOutput_Set_Raw_PWM(pwm_test_value);
//
//		            // EXIT: Press ENTER to exit test mode
//		            if (key == KEY_ENTER) {
//		                AnalogOutput_Calibration_Bypass(false);
//		                Keyboard_SetMode(MODE_NORMAL);
//		            }
