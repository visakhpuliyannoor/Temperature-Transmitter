...
/* ================= Debounce + key event engine =====================
 * Visakh - replaces the old blocking Read_key().
 *
 * Read_Keys_Stable() : level that has been steady for KEY_DEBOUNCE_MS.
 *                      A single bounce/glitch sample can never be latched.
 * Keys_GetEvent()    : returns a key mask ONLY on the tick an action is due
 *                      - once on a fresh press / new combination
 *                      - then repeatedly for UP/DOWN after KEY_REPEAT_DELAY_MS
 *                      - NO_KEY at all other times
 *                      Everything downstream keeps working unchanged, but now
 *                      fires exactly once per press instead of once per loop.
 * =================================================================== */
#define KEY_DEBOUNCE_MS       30    /* steady time before a change is accepted */
#define KEY_REPEAT_DELAY_MS   600   /* hold this long before UP/DOWN repeats   */
#define KEY_REPEAT_SLOW_MS    200   /* repeat period, 5 counts/s               */
#define KEY_REPEAT_FAST_MS    50    /* repeat period after acceleration        */
#define KEY_ACCEL_AFTER       6     /* repeats at slow rate before speeding up */

static uint8_t  key_stable    = NO_KEY;
static uint8_t  key_candidate = NO_KEY;
static uint32_t key_change_ts = 0;
static uint32_t key_press_ts  = 0;
static uint32_t key_repeat_ts = 0;
static uint16_t key_repeat_n  = 0;
...
uint8_t Read_Keys_Stable(void)
{
	...
	if (raw != key_candidate) {
		key_candidate = raw;          /* level changed: restart the timer */
		key_change_ts = now;
	}
	else if ((now - key_change_ts) >= KEY_DEBOUNCE_MS) {
		key_stable = key_candidate;   /* steady long enough: accept it */
	}
...
{
	static uint8_t last_stable = NO_KEY;

	uint8_t  s   = Read_Keys_Stable();
	uint32_t now = HAL_GetTick();
	uint8_t  evt = NO_KEY;

	if (s != last_stable) {
		/* press edge, release edge, or a new key combination */
		last_stable   = s;
		key_press_ts  = now;
		key_repeat_ts = now;
		key_repeat_n  = 0;
		...
		if (s != NO_KEY) {
			evt = s;                  /* fire ONCE for this press */
		}
	}
	else if ((s == KEY_UP) || (s == KEY_DOWN)) {
		/* auto-repeat, single key only - never for combinations */
		if ((now - key_press_ts) >= KEY_REPEAT_DELAY_MS) {

			uint32_t period = (key_repeat_n >= KEY_ACCEL_AFTER)
			                ? KEY_REPEAT_FAST_MS : KEY_REPEAT_SLOW_MS;

			if ((now - key_repeat_ts) >= period) {
				key_repeat_ts = now;
				if (key_repeat_n < 0xFFFF) key_repeat_n++;

				/* hand acceleration over to your existing Fast_value_Inc/Dec */
				Keyboard.Fast_Value_Change =
					(key_repeat_n >= KEY_ACCEL_AFTER) ? 1 : 0;
...
static void SetDownKeyPressed(void)
{
	...
	while ((HAL_GetTick() - start_time) < 5000) {
		// Direct GPIO read (no delay!)
		if (Read_Keys_Stable() != (KEY_SET | KEY_DOWN)) {
			Keyboard.KeyPressed = NO_KEY;
			return;  // Keys released early
		}
		HAL_Delay(100);  // Only one delay per loop
...
static void SetUpKeyPressed(void)
{
	...
	while ((HAL_GetTick() - start_time) < 5000) {
		if (Read_Keys_Stable() != (KEY_SET | KEY_UP)) {
			Keyboard.KeyPressed = NO_KEY;
			return;
		}
		HAL_Delay(100);
...
static void UpEnterKeyPressed(void)
{
	...
	while ((HAL_GetTick() - start_time) < 10000) {  // 10 seconds
		if (Read_Keys_Stable() != (KEY_UP | KEY_ENTER)) {
			Keyboard.KeyPressed = NO_KEY;
			return;
		}
		HAL_Delay(100);
...
static void DownEnterPWMTestKeyPressed(void)
{
	...
	while ((HAL_GetTick() - start_time) < 5000) {
		if (Read_Keys_Stable() != (KEY_DOWN | KEY_ENTER)) {
			Keyboard.KeyPressed = NO_KEY;
			return;
		}
		HAL_Delay(100);
...
static void EnterKeyPressed(void)
{
	/* No wait-for-release here any more.
	 * Keys_GetEvent() already guarantees ONE event per physical press, and the
	 * old loop exited on the first bounce gap (causing the 2-3 menu skip) and
	 * blocked the K2+K4 / K3+K4 combos whenever K4 was pressed first. */
	// Handle ENTER key for mode transitions
	switch (current_mode) {
	case MODE_INPUT_CAL:
		current_mode = MODE_INPUT_CAL_LOW;
